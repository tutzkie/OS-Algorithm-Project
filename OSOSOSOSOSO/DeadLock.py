...
class DeadlockAvoidance:
    def __init__(self, num_processes: int, num_resources: int, allocation: list[list[int]], max_demand: list[list[int]], available: list[int]):
        ...
        for i in range(self.n):
            for j in range(self.m):
                if allocation[i][j] > max_demand[i][j]:
                    raise ValueError(f"allocation[{i}][{j}] cannot exceed max_demand[{i}][{j}].")
    ...
    def _compute_need(self) -> list[list[int]]:
        return [[self.max_demand[i][j] - self.allocation[i][j] for j in range(self.m)] for i in range(self.n)]
    ...
    def is_safe(self) -> SafetyResult:
        ...
        while len(sequence) < self.n:
            ...
            if not found:
                return SafetyResult(safe=False, sequence=[], steps=steps + [{"action": "unsafe", "unfinished": [p for p, f in enumerate(finish) if not f]}])
    ...
    def request_resources(self, process_id: int, request: list[int]) -> RequestResult:
        ...
        if any(request[j] > self.need[process_id][j] for j in range(self.m)):
            return RequestResult(False, f"Denied: request exceeds need for P{process_id}.")

        if any(request[j] > self.available[j] for j in range(self.m)):
            return RequestResult(False, f"Denied: not enough available resources for P{process_id}.")
        ...
        if result.safe:
            ...
            return RequestResult(True, f"Granted to P{process_id}. System remains safe: {seq_str}.", safe=True, sequence=result.sequence)
        ...
        return RequestResult(False, f"Denied: granting request to P{process_id} would leave an unsafe state.", safe=False)
...
class DeadlockDetection:
    def __init__(self, num_processes: int, num_resources: int, allocation: list[list[int]], current_request: list[list[int]], available: list[int]):
        self.n = num_processes
        self.m = num_resources
        validate_matrix(allocation, self.n, self.m, "allocation")
    ...
    def detect_deadlock(self) -> DetectionResult:
        ...
        return DetectionResult(deadlocked=bool(deadlocked), processes=deadlocked, steps=steps)
...
class WaitForGraph:
    ...
    def to_edges(self) -> list[tuple[int, int]]:
        return [(src, dst) for src in range(self.n) for dst in self.graph[src]]
...
class DeadlockPrevention:
    ...
    def request_resource(self, process_id: int, resource_id: int) -> tuple[bool, str]:
        ...
        return (False, f"Denied: P{process_id} holds up to R{current_highest}; cannot request R{resource_id} (violates resource ordering).")
...
def banker_from_preset(preset: dict | None = None) -> DeadlockAvoidance:
    ...
    return DeadlockAvoidance(data["num_processes"], data["num_resources"], data["allocation"], data["max_demand"], data["available"])

def detection_from_preset(preset: dict | None = None) -> DeadlockDetection:
    data = preset or DETECTION_EXAMPLE
    return DeadlockDetection(data["num_processes"], data["num_resources"], data["allocation"], data["current_request"], data["available"])
...
UI_YELLOW  = "#C8A84B"
UI_WHITE   = "#FFFFFF"
PCOLS      = ["#5B8DF5", "#F5A023", "#22C55E", "#F05070", "#A855F7", "#EAB308", "#06B6D4", "#F97316"]

TAB_DESCS = {
    "banker": "Banker's algorithm — checks whether the system is in a safe state before granting resource requests. A safe sequence means all processes can finish.",
    "detection": "Deadlock detection — simulates resource release to see which processes can still complete. Remaining processes are deadlocked.",
    "rag": "Resource allocation graph — single-instance resources. A cycle in the wait-for graph means the system is deadlocked.",
    "prevention": "Circular-wait prevention — resources must be requested in strictly increasing ID order to prevent cycles.",
}
...
class DeadlockApp(tk.Frame):
    ...
    def _build_ui(self):
        top = tk.Frame(self, bg=UI_BG, padx=24, pady=20)
        top.pack(fill="x")
        tk.Label(top, text="Deadlock Simulator", bg=UI_BG, fg=UI_TEXT, font=("Segoe UI", 18, "bold")).pack(anchor="w")
        tk.Label(top, text="Avoidance, detection, resource allocation graph, and prevention", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 12)).pack(anchor="w", pady=(0, 15))

        tab_row = tk.Frame(top, bg=UI_BG)
        tab_row.pack(fill="x", pady=(0, 15))
        self.tab_btns: dict[str, tk.Button] = {}
        for key, label in [("banker", "Banker's (Avoidance)"), ("detection", "Detection"), ("rag", "RAG / Wait-For"), ("prevention", "Prevention")]:
            btn = tk.Button(tab_row, text=label, font=("Segoe UI", 12, "bold"), relief="flat", padx=18, pady=10, cursor="hand2", command=lambda k=key: self._set_tab(k))
            btn.pack(side="left", padx=(0, 10))
            self.tab_btns[key] = btn

        desc_f = tk.Frame(top, bg=UI_BG2)
        desc_f.pack(fill="x")
        tk.Frame(desc_f, bg=UI_ACCENT, width=4).pack(side="left", fill="y")
        self.desc_var = tk.StringVar(value=TAB_DESCS["banker"])
        tk.Label(desc_f, textvariable=self.desc_var, bg=UI_BG2, fg=UI_TEXT2, font=("Segoe UI", 11), wraplength=900, justify="left", padx=15, pady=12).pack(side="left", fill="x", expand=True)

        self.body = tk.Frame(self, bg=UI_BG, padx=24, pady=15)
    ...
    def _style_tabs(self):
        for key, btn in self.tab_btns.items():
            active = key == self.tab
            btn.configure(bg=UI_BG3 if active else UI_BG, fg=UI_TEXT if active else UI_TEXT2)
    ...
    def _info_banner(self, parent, var: tk.StringVar) -> tk.Label:
        ...
        tk.Frame(box, bg=UI_ACCENT, width=4).pack(side="left", fill="y")
        lbl = tk.Label(box, textvariable=var, bg=UI_BG2, fg=UI_TEXT, font=("Segoe UI", 11), wraplength=860, justify="left", padx=15, pady=12, anchor="w")
    ...
    def _styled_tree(self, parent, cols: list[str], widths: list[int], height: int = 6):
        ...
        wrap.pack(fill="x", pady=(0, 15))
        tree = ttk.Treeview(wrap, columns=cols, show="headings", height=height)
        style = ttk.Style(self)
        style.theme_use("clam")
        style.configure("Dead.Treeview", background=UI_BG2, fieldbackground=UI_BG2, foreground=UI_TEXT, rowheight=28, font=("Segoe UI", 11), borderwidth=0)
        style.configure("Dead.Treeview.Heading", background=UI_BG3, foreground=UI_TEXT2, font=("Segoe UI", 10, "bold"), relief="flat", padding=(5, 5))
        style.map("Dead.Treeview", background=[("selected", UI_ACCENT)], foreground=[("selected", UI_WHITE)])
    ...
    def _entry(self, parent, var: tk.StringVar, width: int = 10) -> tk.Entry:
        e = tk.Entry(parent, textvariable=var, width=width, bg=UI_BG2, fg=UI_TEXT, insertbackground=UI_TEXT, relief="flat", highlightthickness=1, highlightbackground=UI_BORDER, highlightcolor=UI_ACCENT, font=("Segoe UI", 12))
    ...
    def _build_banker_panel(self):
        ...
        tk.Label(ctrl, text="Preset", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        self.banker_preset = tk.StringVar(value=BANKER_EXAMPLE["name"])
        cb = ttk.Combobox(ctrl, textvariable=self.banker_preset, state="readonly", width=30, font=("Segoe UI", 11), values=[BANKER_EXAMPLE["name"], BANKER_UNSAFE_EXAMPLE["name"]])
        cb.pack(side="left", padx=(8, 15))
        cb.bind("<<ComboboxSelected>>", lambda _e: self._on_banker_preset())

        tk.Button(ctrl, text="Apply preset", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._on_banker_preset).pack(side="left", padx=(0, 10))
        tk.Button(ctrl, text="Build model", bg=UI_ACCENT, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._build_banker_model).pack(side="left", padx=(0, 10))
        tk.Button(ctrl, text="Check safety", bg=UI_GREEN, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._run_banker_safety).pack(side="left")

        self._info_banner(p, self.banker_info)

        dims = tk.Frame(p, bg=UI_BG)
        dims.pack(fill="x", pady=(0, 15))
        ...
        for label, var in [("Processes", self.b_n_var), ("Resources", self.b_m_var)]:
            tk.Label(dims, text=label, bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
            tk.Spinbox(dims, from_=1, to=8, width=5, textvariable=var, font=("Segoe UI", 12), command=self._resize_banker_fields).pack(side="left", padx=(6, 18))

        tk.Label(dims, text="Available", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        self.b_avail_var = tk.StringVar(value="3, 3, 2")
        self._entry(dims, self.b_avail_var, width=18)
        ...
        tk.Label(left, text="ALLOCATION (rows = processes)", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 10, "bold")).pack(anchor="w")
        self.b_alloc_text = tk.Text(left, height=7, bg=UI_BG2, fg=UI_TEXT, insertbackground=UI_TEXT, relief="flat", highlightthickness=1, highlightbackground=UI_BORDER, font=("Courier New", 12))
        self.b_alloc_text.pack(fill="both", expand=True, pady=(6, 12))

        right = tk.Frame(grid, bg=UI_BG)
        right.grid(row=0, column=1, sticky="nsew", padx=(10, 0))
        tk.Label(right, text="MAX DEMAND", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 10, "bold")).pack(anchor="w")
        self.b_max_text = tk.Text(right, height=7, bg=UI_BG2, fg=UI_TEXT, insertbackground=UI_TEXT, relief="flat", highlightthickness=1, highlightbackground=UI_BORDER, font=("Courier New", 12))
        self.b_max_text.pack(fill="both", expand=True, pady=(6, 12))

        bottom = tk.Frame(p, bg=UI_BG)
        bottom.pack(fill="x", pady=(12, 0))

        req_row = tk.Frame(bottom, bg=UI_BG2, highlightthickness=1, highlightbackground=UI_BORDER)
        req_row.pack(fill="x", pady=(0, 15))
        inner = tk.Frame(req_row, bg=UI_BG2, padx=15, pady=12)
        inner.pack(fill="x")
        tk.Label(inner, text="Try resource request", bg=UI_BG2, fg=UI_TEXT, font=("Segoe UI", 11, "bold")).pack(side="left", padx=(0, 18))
        tk.Label(inner, text="Process", bg=UI_BG2, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        self.b_req_p_var = tk.StringVar(value="1")
        self._entry(inner, self.b_req_p_var, width=5)
        tk.Label(inner, text="Request vector", bg=UI_BG2, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        self.b_req_vec_var = tk.StringVar(value="0, 2, 0")
        self._entry(inner, self.b_req_vec_var, width=16)
        tk.Button(inner, text="Request", bg=UI_ORANGE, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=18, pady=8, cursor="hand2", command=self._banker_request).pack(side="left")

        step_row = tk.Frame(bottom, bg=UI_BG)
        step_row.pack(fill="x", pady=(0, 10))
        tk.Button(step_row, text="← Prev step", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=12, pady=6, cursor="hand2", command=lambda: self._banker_step(-1)).pack(side="left")
        tk.Button(step_row, text="Next step →", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=12, pady=6, cursor="hand2", command=lambda: self._banker_step(1)).pack(side="left", padx=(10, 0))
        self.b_step_lbl = tk.Label(step_row, text="Safety steps: —", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11))
        self.b_step_lbl.pack(side="left", padx=15)

        self.banker_step_tree = self._styled_tree(bottom, ["Step", "Process", "Work before", "Need", "Work after"], [60, 90, 200, 200, 200], height=5)
        self.banker_state_tree = self._styled_tree(bottom, ["Process", "Allocation", "Max", "Need", "Available"], [90, 200, 200, 200, 140], height=6)
    ...
    def _refresh_banker_state_table(self):
        ...
        for i in range(self.banker.n):
            self.banker_state_tree.insert("", "end", values=(f"P{i}", str(self.banker.allocation[i]), str(self.banker.max_demand[i]), str(self.banker.need[i]), avail if i == 0 else ""))
    ...
    def _render_banker_steps(self):
        ...
        for i, step in enumerate(self.safety_steps):
            if step.get("action") == "unsafe":
                self.banker_step_tree.insert("", "end", values=(i + 1, "—", "—", f"Blocked: {step.get('unfinished')}", "—"))
            else:
                self.banker_step_tree.insert("", "end", values=(i + 1, f"P{step['process']}", str(step["work_before"]), str(step["need"]), str(step["work_after"])))
    ...
    def _build_detection_panel(self):
        ...
        ctrl = tk.Frame(p, bg=UI_BG)
        ctrl.pack(fill="x", pady=(0, 15))
        tk.Button(ctrl, text="Load example", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=lambda: self._load_detection_preset(DETECTION_EXAMPLE)).pack(side="left", padx=(0, 10))
        tk.Button(ctrl, text="Build model", bg=UI_ACCENT, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._build_detection_model).pack(side="left", padx=(0, 10))
        tk.Button(ctrl, text="Detect deadlock", bg=UI_RED, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._run_detection).pack(side="left")
        ...
        for label, var in [("Processes", self.d_n_var), ("Resources", self.d_m_var)]:
            tk.Label(dims, text=label, bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
            tk.Spinbox(dims, from_=1, to=8, width=5, textvariable=var, font=("Segoe UI", 12)).pack(side="left", padx=(6, 18))
        tk.Label(dims, text="Available", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        self.d_avail_var = tk.StringVar(value="0, 0, 0")
        self._entry(dims, self.d_avail_var, width=18)
        ...
        left = tk.Frame(grid, bg=UI_BG)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 10))
        tk.Label(left, text="ALLOCATION", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 10, "bold")).pack(anchor="w")
        self.d_alloc_text = tk.Text(left, height=7, bg=UI_BG2, fg=UI_TEXT, insertbackground=UI_TEXT, relief="flat", highlightthickness=1, highlightbackground=UI_BORDER, font=("Courier New", 12))
        self.d_alloc_text.pack(fill="both", expand=True, pady=(6, 12))

        right = tk.Frame(grid, bg=UI_BG)
        right.grid(row=0, column=1, sticky="nsew", padx=(10, 0))
        tk.Label(right, text="CURRENT REQUEST", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 10, "bold")).pack(anchor="w")
        self.d_req_text = tk.Text(right, height=7, bg=UI_BG2, fg=UI_TEXT, insertbackground=UI_TEXT, relief="flat", highlightthickness=1, highlightbackground=UI_BORDER, font=("Courier New", 12))
        self.d_req_text.pack(fill="both", expand=True, pady=(6, 12))

        step_row = tk.Frame(p, bg=UI_BG)
        step_row.pack(fill="x", pady=(12, 10))
        tk.Button(step_row, text="← Prev", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=12, pady=6, cursor="hand2", command=lambda: self._det_step(-1)).pack(side="left")
        tk.Button(step_row, text="Next →", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=12, pady=6, cursor="hand2", command=lambda: self._det_step(1)).pack(side="left", padx=(10, 0))
        self.d_step_lbl = tk.Label(step_row, text="Detection steps: —", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11))
        self.d_step_lbl.pack(side="left", padx=15)

        self.det_step_tree = self._styled_tree(p, ["Step", "Process", "Work before", "Request", "Work after"], [60, 90, 220, 220, 220], height=8)
    ...
    def _render_det_steps(self):
        ...
        for i, step in enumerate(self.det_steps):
            self.det_step_tree.insert("", "end", values=(i + 1, f"P{step['process']}", str(step["work_before"]), str(step["request"]), str(step["work_after"])))
    ...
    def _build_rag_panel(self):
        ...
        ctrl = tk.Frame(p, bg=UI_BG)
        ctrl.pack(fill="x", pady=(0, 15))
        tk.Button(ctrl, text="Load example", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=lambda: self._load_rag_preset(RAG_EXAMPLE)).pack(side="left", padx=(0, 10))
        tk.Button(ctrl, text="Apply", bg=UI_ACCENT, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._apply_rag).pack(side="left", padx=(0, 10))
        tk.Button(ctrl, text="Detect deadlock", bg=UI_RED, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._run_rag_detection).pack(side="left")
        ...
        tk.Label(cfg, text="Processes", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        tk.Spinbox(cfg, from_=1, to=8, width=5, textvariable=self.r_n_var, font=("Segoe UI", 12), command=self._rebuild_rag_table).pack(side="left", padx=(6, 18))
        tk.Label(cfg, text="Resources", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        tk.Spinbox(cfg, from_=1, to=8, width=5, textvariable=self.r_m_var, font=("Segoe UI", 12), command=self._rebuild_rag_table).pack(side="left", padx=(6, 18))
        ...
        table_wrap.grid(row=0, column=0, sticky="nsew", padx=(0, 15))
        ...
        self.rag_canvas.pack(fill="both", expand=True, padx=12, pady=12)
    ...
    def _rebuild_rag_table(self):
        ...
        tk.Label(self.rag_table_frame, text="RESOURCE HOLDERS", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 8))
        for r in range(self.r_m_var.get()):
            row = tk.Frame(self.rag_table_frame, bg=UI_BG)
            row.pack(fill="x", pady=4)
            tk.Label(row, text=f"R{r}", bg=UI_BG, fg=PCOLS[r % len(PCOLS)], font=("Segoe UI", 12, "bold"), width=4).pack(side="left")
            var = tk.StringVar(value="—")
            self.rag_holder_vars.append(var)
            ttk.Combobox(row, textvariable=var, width=10, font=("Segoe UI", 11), state="readonly", values=["—"] + [f"P{p}" for p in range(self.r_n_var.get())]).pack(side="left")

        tk.Label(self.rag_table_frame, text="PROCESS WAITING FOR", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(18, 8))
        for pidx in range(self.r_n_var.get()):
            row = tk.Frame(self.rag_table_frame, bg=UI_BG)
            row.pack(fill="x", pady=4)
            tk.Label(row, text=f"P{pidx}", bg=UI_BG, fg=PCOLS[pidx % len(PCOLS)], font=("Segoe UI", 12, "bold"), width=4).pack(side="left")
            var = tk.StringVar(value="—")
            self.rag_wait_vars.append(var)
            ttk.Combobox(row, textvariable=var, width=10, font=("Segoe UI", 11), state="readonly", values=["—"] + [f"R{r}" for r in range(self.r_m_var.get())]).pack(side="left")
    ...
    def _draw_rag(self, cycle: list[int] | None = None):
        ...
        p_pos = {i: (90, 50 + i * (H - 100) / max(n - 1, 1)) for i in range(n)}
        r_pos = {i: (W - 90, 50 + i * (H - 100) / max(m - 1, 1)) for i in range(m)}
        ...
        for src, dst in wfg.to_edges():
            ...
            c.create_line(x0 + 22, y0, x1 - 22, y1, fill=color, width=3, arrow=tk.LAST)
            mx, my = (x0 + x1) / 2, (y0 + y1) / 2 - 20
            c.create_text(mx, my, text=f"P{src}→P{dst}", fill=color, font=("Segoe UI", 10, "bold"))

        for r, (x, y) in r_pos.items():
            holder = self.rag.holders[r]
            c.create_rectangle(x - 20, y - 20, x + 20, y + 20, fill=UI_BG3, outline=UI_TEXT2, width=2)
            c.create_text(x, y, text=f"R{r}", fill=UI_TEXT, font=("Segoe UI", 11, "bold"))
            if holder >= 0:
                hx, hy = p_pos[holder]
                c.create_line(x - 20, y, hx + 22, hy, fill=UI_GREEN, width=3, arrow=tk.LAST)

        for p, (x, y) in p_pos.items():
            waiting = self.rag.waiting[p]
            col = PCOLS[p % len(PCOLS)]
            c.create_oval(x - 22, y - 22, x + 22, y + 22, fill=col, outline=col)
            c.create_text(x, y, text=f"P{p}", fill=UI_WHITE, font=("Segoe UI", 11, "bold"))
            if waiting >= 0:
                wx, wy = r_pos[waiting]
                c.create_line(x + 22, y, wx - 20, wy, fill=UI_ORANGE, width=3, dash=(6, 4), arrow=tk.LAST)

        c.create_text(W / 2, 20, text="Processes (left) · Resources (right) · Yellow/red = wait-for edges", fill=UI_TEXT3, font=("Segoe UI", 10))

    def _build_prevention_panel(self):
        p = self.panels["prevention"]
        self.prev_info = tk.StringVar(value="Request resources in increasing ID order. Lower/equal IDs are denied.")

        ctrl = tk.Frame(p, bg=UI_BG)
        ctrl.pack(fill="x", pady=(0, 15))
        tk.Label(ctrl, text="Processes", bg=UI_BG, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        self.prev_n_var = tk.IntVar(value=3)
        tk.Spinbox(ctrl, from_=1, to=8, width=5, textvariable=self.prev_n_var, font=("Segoe UI", 12), command=self._reset_prevention).pack(side="left", padx=(6, 18))
        tk.Button(ctrl, text="Reset", bg=UI_BG3, fg=UI_TEXT, font=("Segoe UI", 11, "bold"), relief="flat", padx=15, pady=8, cursor="hand2", command=self._reset_prevention).pack(side="left")

        self._info_banner(p, self.prev_info)

        req = tk.Frame(p, bg=UI_BG2, highlightthickness=1, highlightbackground=UI_BORDER)
        req.pack(fill="x", pady=(0, 15))
        inner = tk.Frame(req, bg=UI_BG2, padx=15, pady=12)
        inner.pack(fill="x")
        tk.Label(inner, text="Process", bg=UI_BG2, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left")
        self.prev_p_var = tk.StringVar(value="0")
        self._entry(inner, self.prev_p_var, width=5)
        tk.Label(inner, text="Resource ID", bg=UI_BG2, fg=UI_TEXT2, font=("Segoe UI", 11)).pack(side="left", padx=(10, 0))
        self.prev_r_var = tk.StringVar(value="2")
        self._entry(inner, self.prev_r_var, width=5)
        tk.Button(inner, text="Request", bg=UI_GREEN, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=18, pady=8, cursor="hand2", command=self._prev_request).pack(side="left", padx=(15, 10))
        tk.Button(inner, text="Release", bg=UI_ORANGE, fg=UI_WHITE, font=("Segoe UI", 11, "bold"), relief="flat", padx=18, pady=8, cursor="hand2", command=self._prev_release).pack(side="left")

        self.prev_state_tree = self._styled_tree(p, ["Process", "Highest resource held"], [140, 260], height=6)

        log_wrap = tk.Frame(p, bg=UI_BORDER, padx=1, pady=1)
        log_wrap.pack(fill="both", expand=True, pady=(12, 0))
        self.prev_log = tk.Text(log_wrap, height=10, bg=UI_BG2, fg=UI_TEXT2, insertbackground=UI_TEXT, relief="flat", font=("Courier New", 12), state="disabled")
